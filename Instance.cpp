...
std::string Instance::to_string() const{
	std::string inst_str = "2D-VS Instance with "+std::to_string(_num_vehicles)+" vehicles.\n";
	inst_str += "Depots: ";
...
Instance::Instance(std::string file){
	...
	//warn if file is bad!
	if(!infile){
		std::cerr<<"Warning: Could not load the file "<<file<<"! Creating empty instance."<<std::endl;
		return;
	}	
	...
	if(_num_vehicles!=_depotPositions.size())
		std::cerr<< "Warning: Number of vehicles and given depot positions does not fit!" <<std::endl;
}


void Instance::_parse_line(std::string &line){
	...
	if( boost::regex_match (line.c_str(),cm,vehicles)){
		...
		if( _num_vehicles >0)
			std::cerr<<"Warning: Multiple lines for number of vehicles!"<<std::endl;
			
		 _num_vehicles = std::stoi(cm[1]);
		 return;
	}
	...
	if(boost::regex_match (line.c_str(),cm,depot_start)){
		...
		while(boost::regex_match (line.c_str(),cm,successive_depot)){
			_depotPositions.push_back(std::array<int, 2>{{std::stoi(cm[1]),std::stoi(cm[2])}});
			line = cm[3];
		}
		return;
	}
	...
	if( boost::regex_match (line.c_str(),cm,forward_job)){
		Job j(_jobs.size()+1,std::stoi(cm[1]),std::stoi(cm[2]),
						   std::stoi(cm[3]),std::stoi(cm[4]));
		_jobs.push_back( j );
		return;
	}
	...
	if( boost::regex_match (line.c_str(),cm,backward_job)){
		Job j(_jobs.size()+1,std::stoi(cm[3]),std::stoi(cm[4]),
						   std::stoi(cm[1]),std::stoi(cm[2]));
		_jobs.push_back( j );
		return;
	}
	//if nothing was matching, return false				
	std::cerr<<"Warning: Could not parse this line: \""<<line<<"\""<<std::endl;
...
void Instance::generate_random_depots(int min_x, int max_x, 
	...
	while( _depotPositions.size() < _num_vehicles)
		_depotPositions.push_back( std::array<int,2>{{dist_x(rng),dist_y(rng)}});
}
...
bool Instance::verify(Tours& t) const{
	...
	for(auto j = _jobs.begin(); j!=_jobs.end(); ++j )
		if(!t.contains(&*j)){
			std::cout<<"Job "<<*j<<" missing!"<<std::endl;
			return false;
		}
	...
	if(_jobs.size()!=t.num_jobs()){
		std::cout<<"Wrong number of jobs. Needed "<<_jobs.size()
					<<" found "<<t.num_jobs()<<std::endl;
		return false;
	}
	...
	for(uint i=0; i<t.num_tours(); ++i){
		...
		std::tie(job1, time1) = t[i].front();
		if( dist_inf(job1->get_alpha(),_depotPositions[i]) > time1+EPS  ){
			std::cout<< "Job "<<*job1<<" not reachable from depot"<<std::endl;
			std::cout<<"starting times: "<<time1<<std::endl;
			std::cout<< "dist: "<<dist_inf(job1->get_alpha(),_depotPositions[i])<<std::endl;
			std::cout<<"difference: "<<time1-dist_inf(job1->get_alpha(),_depotPositions[i])<<std::endl;
			return false;
		}
			
		for(uint j=1; j<t[i].size(); ++j){
			double time2; const Job* job2;
			std::tie(job1, time1) = t[i][j-1];
			std::tie(job2, time2) = t[i][j];		
			if(dist_inf(job1->get_beta(),job2->get_alpha())+job1->length() > (time2 - time1 )+EPS){
				std::cout<< "Job "<<*job2<<" not reachable after job "<<*job1<<std::endl;
				std::cout<<"starting times: "<<time1 << " and "<<time2<<std::endl;
				std::cout<<time1<< " + "<<job1->length()<<" + "<<dist_inf(job1->get_beta(),job2->get_alpha()) <<" - EPS > "<<time2<<std::endl;
				return false;
			}	
		}
...
double Instance::makespan(Tours& t) const{
	...
	for(uint i=0; i < t.num_tours(); ++i){
	    if(0==t[i].size())
	    	continue;
		const Job* j = std::get<0>(t[i].back());
	 	double time = std::get<1>(t[i].back()) + j->length();
...
Tours Instance::get_MIP_solution() const{
